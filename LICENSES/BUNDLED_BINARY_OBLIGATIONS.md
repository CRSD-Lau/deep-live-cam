...
| NVIDIA CUDA/cuDNN runtime DLLs from pinned `torch==2.11.0+cu128` | CUDA 12/cuDNN 9 DLLs copied into the PyInstaller `_internal` payload for ONNX Runtime GPU loading | Keep exact DLL names in `WINDOWS_BUNDLE_MANIFEST.md` and ship the wheel's `LICENSE`, `NOTICE`, and metadata under `LICENSES/THIRD_PARTY_LICENSES/torch-2.11.0_cu128/`. |
