...
The Windows installer intentionally does not bundle model/checkpoint files, ffmpeg/ffprobe, OBS, NVIDIA drivers, the CUDA Toolkit installer, or TensorRT system runtime installers. It does bundle selected CUDA/cuDNN runtime DLLs from the CUDA-enabled PyTorch wheel so `onnxruntime-gpu` can load in the frozen app. If a future release bundles additional NVIDIA components, add their exact license, source, build configuration, and redistribution permission evidence here before publishing.
