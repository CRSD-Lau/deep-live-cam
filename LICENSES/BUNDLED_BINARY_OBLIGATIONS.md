...
| ONNX Runtime GPU or ONNX Runtime DirectML, TensorFlow, OpenCV, NumPy/SciPy, Pillow, PySide6 | Native DLLs and `.pyd` extension modules | Preserve bundled notice/license files collected from the selected runtime wheel. Review native-library notices in the generated `WINDOWS_BUNDLE_MANIFEST.md` and installed `_internal` tree before publishing. CUDA and DirectML are separate, mutually exclusive build profiles. |
| NVIDIA CUDA/cuDNN runtime DLLs from the CUDA-enabled PyTorch wheel | CUDA 12/cuDNN 9 DLLs copied into the PyInstaller `_internal` payload for ONNX Runtime GPU loading | Keep exact DLL names in `WINDOWS_BUNDLE_MANIFEST.md`. Before public distribution, confirm redistribution rights and include any NVIDIA/PyTorch wheel notices required by legal review. |
...
The Windows installer intentionally does not bundle model/checkpoint files, ffmpeg/ffprobe, OBS, GPU drivers, the CUDA Toolkit installer, or TensorRT system runtime installers. The CUDA profile bundles selected CUDA/cuDNN runtime DLLs from the CUDA-enabled PyTorch wheel so `onnxruntime-gpu` can load in the frozen app. The DirectML test profile instead bundles `onnxruntime-directml` and `DirectML.dll`; it does not bundle CUDA runtime DLLs. Re-run legal review for both profiles before promoting DirectML to a public release asset.
