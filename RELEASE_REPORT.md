...
```text
C:\Projects\deep-live-cam\build\windows\release-assets\2.1.7
```
...
powershell -ExecutionPolicy Bypass -File build\windows\test_installer.ps1 -AppVersion 2.1.7
...
`build/windows/release-assets/2.1.7/`, and validates that curated upload folder.
...
```powershell
python tools\validate_windows_release_artifacts.py --app-version 2.1.7 --require-git-ref-source
```
...
The assembled files are written under `build/windows/release-assets/2.1.7/`
...
```powershell
python tools\validate_windows_release_artifacts.py --app-version 2.1.7 --require-git-ref-source --release-assets-dir build\windows\release-assets\2.1.7
```
...
```text
C:\Projects\deep-live-cam\build\windows\installer\DeepLiveCamStudio-2.1.7-x64-setup.exe
```
...
```powershell
Get-FileHash build\windows\installer\DeepLiveCamStudio-2.1.7-x64-setup.exe -Algorithm SHA256
```
...
```powershell
powershell -ExecutionPolicy Bypass -File build\windows\package_source.ps1 -AppVersion 2.1.7 -FromWorkingTree
```
...
- Current installer smoke test passed against `DeepLiveCamStudio-2.1.7-x64-setup.exe` after the silent-uninstall model-preservation fix.
