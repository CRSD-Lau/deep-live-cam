...
Report generated from the local Windows packaging workspace on 2026-05-18.
...
The sidecar is generated by `build/windows/package_installer.ps1` and is the authoritative local hash record for the installer. `RELEASE_VERIFICATION.md` records the latest local hash and size after packaging. Confirm the sidecar matches before publishing:
...
- Current packaged runtime preflight passed against `dist\DeepLiveCamStudio`.
...
- Latest local release-candidate wrapper pass rebuilt and smoke-tested the installer, verified the installer hash sidecar, rebuilt the draft working-tree source archive, and validated the uploadable artifact set.
- `RELEASE_VERIFICATION.md` and the generated `.sha256` sidecars are the authoritative hash records for the latest local artifact set; do not embed installer hashes in documents that are themselves packaged into the installer.
