...
- `DeepLiveCamStudio-2.2.1-x64-setup.exe` — NVIDIA/CUDA installer
- `DeepLiveCamStudio-2.2.1-DirectML-x64-portable.zip` — AMD/Intel DirectML
...
1. Create annotated tag `v2.2.1` on the fully verified merged commit, then
...
8. Close issue #8 with privacy-preserving physical validation evidence, then
   verify there are no open issues, PRs, or security alerts.

## Rollback

If a post-release check fails, keep or restore `v2.2.0` as the latest stable
release, mark `v2.2.1` as a pre-release or draft, and document the failed asset
