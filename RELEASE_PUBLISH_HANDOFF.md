...
The existing draft targets the frozen commit above. Corrected runtime and source assets have replaced the superseded candidate. Preserve all seven frozen binary/source files, their hashes, and the existing `v2.2.3` rollback release. No public 2.2.4 tag exists yet.
...
Complete clean-Windows installation/2.2.3 upgrade, shortcut/model-consent/interactive-uninstall behavior, packaged Preview, physical-camera Live Output start/stop/restart, file/live exclusion and processed output in an independent receiver. Record the exact final executable identities. Source-only synthetic OBS and configured-host installer tests cover their recorded subsets.
...
Use the actual asset directory for both directory options. The artifact validator checks integrity and document structure; the strict summary is the separate manual gate. `Ready to publish without remaining manual gates: YES` is appropriate only after every required current-version check passes.

## Publication order

1. Create and verify the annotated `v2.2.4` tag at the fixed build commit, after manual gates pass.
